...
void average_thrfct_test(const std::vector <int> & randoms, int randindex,
...
void average_thrfct_old_test(const std::vector <int> & randoms, int randindex,
			...
			for (int ts = 0; ts < TEST_COUNT; ts++) {
				...
				try {
					...
					for (int thr = 0; thr < THR_CNT; thr++) {
						...
						thrs[thr] = new std::thread(&average_thrfct_old_test, std::ref(randoms),
							randindex + rand_offset,
							std::ref(sk), std::ref(perm), thr_args[thr], thr_val[thr],
							std::ref(out), std::ref(out_mutex), ROUNDS_PER_THREAD);
...
void multiple_average_predefined_tests(std::string path) {
			...
			for (int k = 0; k < ARR3_CNT; k++) {
				...
				for (int p = 0; p < ARR4_CNT; p++) {
					...
					for (int q = 0; q < ARR5_CNT; q++) {
						...
						rnd += 1;
						if (rnd == TOTAL_CNT)
							average_predefined_test(path_tmp, true);
						else
							average_predefined_test(path_tmp, false);
					}

...
ostream& certFHE_old::operator<<(ostream &out, const SecretKey &c)
  {
...
Ciphertext SecretKey::encrypt(Plaintext &plaintext)
{
	...
	uint64_t n = this->certFHEContext_old->getN();
	uint64_t d = this->certFHEContext_old->getD();
    ...
    Ciphertext c(_encValues,_bitlen,len,*this->certFHEContext_old);
    delete [] vect;
    delete [] _bitlen;    
    delete [] _encValues;
...
void SecretKey::applyPermutation_inplace(const Permutation& permutation)
{
	...
	uint64_t *current_key = new uint64_t[this->certFHEContext_old->getN()];
	
	for(uint64_t i = 0;i<this->certFHEContext_old->getN();i++)
		current_key[i] = 0;

	for(uint64_t i = 0;i<length;i++)
		current_key[s[i]] =1;

	uint64_t *temp = new uint64_t[this->certFHEContext_old->getN()];

	for (int i = 0; i < this->certFHEContext_old->getN(); i++)
...
{
	long size = 0;
	size += sizeof(this->certFHEContext_old);
...
SecretKey::SecretKey(const Context &context)
{
    ...
    this->certFHEContext_old = new certFHE_old::Context(context);

    uint64_t _d = certFHEContext_old->getD();
    uint64_t _n = certFHEContext_old->getN();
...
{
    this->certFHEContext_old = new certFHE_old::Context(*secKey.certFHEContext_old);
...
SecretKey::~SecretKey()
{
    ...
    if (this->certFHEContext_old != nullptr)
    {
        delete this->certFHEContext_old;
        this->certFHEContext_old = nullptr;
    }
}
